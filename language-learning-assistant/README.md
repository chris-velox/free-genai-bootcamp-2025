...
From direct fork of Du'an's repo

```sh
uv init --name led-language-asst -p 3.10

uv pip install streamlit boto3 typing youtube_transcript_api groq python_dotenv
PYTHONPATH=. streamlit run frontend/main.py
```

Get an error that it can't find the the BedrockChat module to install.